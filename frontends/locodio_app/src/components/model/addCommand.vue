...
import type {AddCommandCommand} from "@/api/command/interface/commandCommands";
import {addCommand} from "@/api/command/model/addCommand";
...
const command = ref<AddCommandCommand>({
...
async function add() {
  if (!v$.value.$invalid) {
    isSaving.value = true;
    await addCommand(command.value);
    toaster.add({
      severity: "success",
      summary: "Command added",
      detail: "",
