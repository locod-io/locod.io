...
<template>
  <div v-if="(modelStore.domainModelSelectedId) != 0 && (!modelStore.isDomainModelFinal) && (!modelStore.domainModelLoading)">
...
function deleteAction(event: MouseEvent) {
  ...
  if (target instanceof HTMLElement) {
    confirm.require({
      target: target,
      message: "Are you sure you want to delete '"+modelStore.domainModel?.name+"' ?",
