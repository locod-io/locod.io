    ...
    <td v-if="!isView">
      <div class="p-inputtext-sm">
        <Dropdown optionLabel="type"
                  ...
                  :options="modelStore.lists.fieldTypes"/>
      </div>
    </td>
    <td v-if="!isView">
...
import type {Enum, Field} from "@/api/query/interface/model";
import {useModelStore} from "@/stores/model";
import {useToast} from "primevue/usetoast";
import {required, integer} from "@vuelidate/validators";
import useVuelidate from "@vuelidate/core";
import type {ChangeFieldCommand} from "@/api/command/interface/domainModelCommands";
import {changeField} from "@/api/command/model/changeField";
...
const commandEdit = ref<ChangeFieldCommand>({
  id: props.item.id,
...
async function save() {
  ...
  if (!v$.value.$invalid) {
    isSaving.value = true;
    await changeField(commandEdit.value);
    toaster.add({
      severity: "success",
...
async function deleteItem() {
  isSaving.value = true;
  await deleteField({id: props.item.id});
  toaster.add({
    severity: "success",
