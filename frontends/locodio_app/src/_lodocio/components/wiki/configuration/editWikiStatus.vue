...
<template>
  <tr id="Component_Edit_Wiki_Node_Status"
...
import type {ChangeWikiNodeStatusCommand} from "@/_lodocio/api/command/wiki/changeWikiNodeStatus";
import {changeWikiNodeStatus} from "@/_lodocio/api/command/wiki/changeWikiNodeStatus";
import {deleteWikiNodeStatus} from "@/_lodocio/api/command/wiki/deleteWikiNodeStatus";
...
const commandEdit = ref<ChangeWikiNodeStatusCommand>({
  id: props.item.id,
...
async function save() {
  ...
  if (!vStatusEdit$.value.$invalid) {
    isSaving.value = true;
    await changeWikiNodeStatus(commandEdit.value);
    toaster.add({
      severity: "success",
...
async function deleteItem() {
  isSaving.value = true;
  await deleteWikiNodeStatus({id: props.item.id});
  toaster.add({
    severity: "success",
