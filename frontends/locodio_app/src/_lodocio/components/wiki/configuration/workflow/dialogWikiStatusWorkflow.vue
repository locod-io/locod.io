...
<template>
  <div id="dialogWikiStatusWorkflow" style="min-width: 800px;">
...
import {getWikiNodeStatusWorkflow} from "@/_lodocio/api/query/wiki/getWikiNodeStatusWorkflow";
import {useAppStore} from "@/stores/app";
import {useDocProjectStore} from "@/_lodocio/stores/project";
import type {WikiNodeStatusWorkflowCommand} from "@/_lodocio/api/command/wiki/saveWikiNodeStatusWorkflow";
import {saveWikiNodeStatusWorkflow} from "@/_lodocio/api/command/wiki/saveWikiNodeStatusWorkflow";
...
async function getWorkflow() {
  let workflow = await getWikiNodeStatusWorkflow(projectStore.selectedWikiId);
...
async function saveWorkflow() {
  isSaving.value = true;
  await saveWikiNodeStatusWorkflow(projectStore.selectedWikiId, command.value);
  toaster.add({
    severity: "success",
...
const command = computed((): WikiNodeStatusWorkflowCommand => {
  let result = [];
