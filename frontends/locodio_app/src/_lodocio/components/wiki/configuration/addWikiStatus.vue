...
import type {AddWikiNodeStatusCommand} from "@/_lodocio/api/command/wiki/addWikiNodeStatus";
import {addWikiNodeStatus} from "@/_lodocio/api/command/wiki/addWikiNodeStatus";
...
const commandAdd = ref<AddWikiNodeStatusCommand>({
  wikiId: projectStore.selectedWikiId,
  name: '',
  color: 'CCCCCC',
  isStart: false,
  isFinal: false,
...
async function save() {
  ...
  if (!vStatusAdd$.value.$invalid) {
    isSaving.value = true;
    await addWikiNodeStatus(commandAdd.value);
    toaster.add({
      severity: "success",
