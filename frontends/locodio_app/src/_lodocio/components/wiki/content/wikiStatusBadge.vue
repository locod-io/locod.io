  ...
  <div id="statusBadgeNode" class="flex gap-2">
    <div class="bg-gray-200 rounded-full flex flex-none w-46 dark:bg-gray-600">
      ...
      <div @click="selectNode"
        ...
        {{ wikiStore.wiki.code }}-{{ nodeRef.artefactId }}
      </div>
...
import {changeStatusNode} from "@/_lodocio/api/command/wiki/changeStatusNode";
...
const availableStatus = computed(() => {
  let status = [];
  if (wikiStore.wiki && wikiStore.wiki.workflow) {
    for (const statusItem of wikiStore.wiki.workflow) {
...
async function changeStatus(statusId: number) {
  ...
  isSaving.value = true;
  let command = {id: nodeRef.value.id, wikiNodeStatusId: statusId};
