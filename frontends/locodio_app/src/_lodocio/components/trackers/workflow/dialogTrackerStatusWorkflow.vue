...
<template>
  <div id="dialogTrackerStatusWorkflow" style="min-width: 800px;">
...
import {getTrackerNodeStatusWorkflow} from "@/_lodocio/api/query/tracker/getTrackerNodeStatusWorkflow";
import {useAppStore} from "@/stores/app";
import {useDocProjectStore} from "@/_lodocio/stores/project";
import type {TrackerNodeStatusWorkflowCommand} from "@/_lodocio/api/command/tracker/saveTrackerNodeStatusWorkflow";
import {saveTrackerNodeStatusWorkflow} from "@/_lodocio/api/command/tracker/saveTrackerNodeStatusWorkflow";
...
async function getWorkflow() {
  let workflow = await getTrackerNodeStatusWorkflow(projectStore.selectedTrackerId);
...
async function saveWorkflow() {
  isSaving.value = true;
  await saveTrackerNodeStatusWorkflow(projectStore.selectedTrackerId, command.value);
  toaster.add({
    severity: "success",
...
const command = computed((): TrackerNodeStatusWorkflowCommand => {
  let result = [];
